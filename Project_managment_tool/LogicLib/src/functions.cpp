...
void change_finish_date_project(std::vector<Project> &proj_vec) { //
    ...
    if (new_date < std::chrono::year_month_day(std::chrono::floor<std::chrono::days>(std::chrono::system_clock::now()))) {
        std::cout << "Worn";
        return;
    }
...
void change_finish_date_personal_task(Employee &usr, std::vector<PersonalTask> &ptask_vec) { //
    ...
    if (task_it->is_task_overdue()) {
        task_it->change_overdue_status(0);
    }
...
void complete_work_task(std::vector<Employee> &empl_vec, std::vector<WorkTask> &wtask_vec,
    ...
    std::vector<int> empl_on_task_id_vec = task_it->get_employees_id();

    for (auto x:empl_on_task_id_vec) {
        auto empl_it = std::find_if(empl_vec.begin(), empl_vec.end(),
                                    [x](Employee empl){return empl.get_id() == x;});

        int count = empl_it->get_work_tasks().size();

        empl_it->set_kpi(empl_it->get_kpi() + 1/count);
    }
...
void complete_work_task(Employee &cur_user, std::vector<Employee> &empl_vec, std::vector<WorkTask> &wtask_vec,
                        std::vector<Project> &proj_vec) {
    int task_id, project_id, done_count = 0, count;
...
void check_projects(std::vector<Employee> &empl_vec, std::vector<WorkTask> &task_vec, std::vector<Project> &proj_vec) {
    ...
    for (auto tmp:proj_vec){
        ...
        auto empl_id = tmp.get_employees_id();
