...
#include <fstream>
#include "../LogicLib/include/db_manager.h"
#include "../LogicLib/include/role_class.h"
...
std::vector<std::tuple<int, std::string, std::vector<int>,
    ...
    for (auto x:in_vec) {
        auto tmp = std::make_tuple(x.get_id(), x.get_name(), x.get_tasks(),
                                   x.get_date_start(), x.get_date_finish(), x.get_task_count(),
                                   x.get_done_task_count(), x.is_completed(), x.get_employees_id());
